...
# Railroad Diagram — `chain_expression` (lexicon: alpha)
...
flowchart LR
    ...
    subgraph SC["selector_call"]
        direction LR
        d(["$$"]):::term --> lp1(["("]):::term --> qs["quoted_selector"]:::rule --> rp1([")"]):::term
    end
...
(`$$ ( quoted_selector )` — mandatory, exactly one), and arrives at the
...
the zero-operation case (`$$('#wheel');` with no verbs). Inside
`operation_call`, the dotted bypass around `argument_list` renders the `?`
optionality, so `#spin()` and `#scale('#f00')` are both covered by
one box.
