<!-- GENERATED FILE — do not edit.
   ...
   to change the lexicon, edit candidates/phi_beta.json and re-render. -->

# Railroad Diagram — `chain_expression` (lexicon: beta)
...
flowchart LR
    ...
    subgraph SC["selector_call"]
        direction LR
        d(["&Q"]):::term --> lp1(["("]):::term --> qs["quoted_selector"]:::rule --> rp1([")"]):::term
    end
...
(`&Q ( quoted_selector )` — mandatory, exactly one), and arrives at the
...
the zero-operation case (`&Q('~wheel');` with no verbs). Inside
`operation_call`, the dotted bypass around `argument_list` renders the `?`
optionality, so `~brofar()` and `~flertum('#f00')` are both covered by
one box.
